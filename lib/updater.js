...
Updater.prototype = {
    ...
    detach: function(mount_point, callback) {
        exec('hdiutil detach ' + escapeshell(mount_point), function(err){
            if (!err) console.log('detached ' + mount_point);
    ...
    hideOriginal: function(app, callback) {
        var filename = app + '.app';
        exec('mv ' + escapeshell(appPath + '/' + filename) + ' ' + escapeshell(appPath + '/.' + filename), callback);
    },
