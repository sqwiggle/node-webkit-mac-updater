...
Updater.prototype = {
        ...
        try {
            this.download(this.config.source, location, function(){
                ...
                self.mount(location, self.config.dmg_name, function(mount_point){
                    ...
                    self.hideOriginal(self.app_name, function(err){
                        if (err) throw err;
                        console.log('original application hidden');
                    
                        self.copyUpdate(self.config.app_name, mount_point, appPath, function(err){
                            if (err) throw err;
    ...
    unmount: function(mount_point, callback) {
        exec('hdiutil unmount ' + escapeshell(mount_point), callback);
    },
